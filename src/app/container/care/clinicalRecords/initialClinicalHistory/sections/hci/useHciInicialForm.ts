...
import {
  useCreateAnalisisDiagnosticosPlanHCInicial,
  useUpdateAnalisisDiagnosticosPlanHCInicial,
} from "@/core/hooks/care/hciInicial/useSaveAnalisisDiagnosticosPlan";
import {
  useCreateHCInicial,
  useUpdateHCInicial,
} from "@/core/hooks/care/hciInicial/useSaveHCInicial";
import {
  useCreateObjetivoHCInicial,
  useUpdateObjetivoHCInicial,
} from "@/core/hooks/care/hciInicial/useSaveObjetivo";
import {
  useCreateSignosVitalesHCInicial,
  useUpdateSignosVitalesHCInicial,
} from "@/core/hooks/care/hciInicial/useSaveSignosVitales";
import {
  useCreateSubjetivoHCInicial,
  useUpdateSubjetivoHCInicial,
} from "@/core/hooks/care/hciInicial/useSaveSubjetivo";
...
interface UseHciInicialFormArgs {
...
export const useHciInicialForm = ({
  ...
  const [subjective, setSubjective] =
    useState<SubjectiveState>(defaultSubjective);
  const [physicalExam, setPhysicalExam] =
    useState<PhysicalExamState>(defaultPhysicalExam);
  const [antecedentes, setAntecedentes] =
    useState<AntecedentesState>(defaultAntecedentes);
  ...
  const isLocked = isClosed && !editMode;
  ...
  const saveSubjetivo = async () => {
    if (isLocked) {
      messageApi.error(
        "Esta historia clínica ya fue clausurada y no admite más cambios.",
      );
      return;
    }
    if (
      !subjective.motivoConsulta.trim() ||
      !subjective.enfermedadActual.trim()
    ) {
      messageApi.error(
    ...
    if (!admissionId) {
      messageApi.error(
        "No se encontró la admisión asociada a esta historia clínica.",
      );
      return;
    }
    ...
    try {
      const saved = subjetivoId
        ? await updateSubjetivo.mutateAsync({
            id: subjetivoId,
            data: { ...subjective, isActive: true },
          })
      ...
      messageApi.error(
        err instanceof Error ? err.message : "No se pudo guardar el subjetivo.",
      );
    } finally {
  ...
  const saveObjetivo = async () => {
    ...
    const missingPhysicalExam = physicalExamFields.some(
      ({ key }) => !physicalExam[key]?.trim(),
    );
    const missingAntecedentes = antecedentesFields.some(
      ({ key }) => !antecedentes[key]?.trim(),
    );
    if (missingPhysicalExam || missingAntecedentes) {
      messageApi.error(
        "Todos los campos del examen físico y los antecedentes son obligatorios.",
      );
      return;
    }
    if (!admissionId) {
    ...
    try {
      ...
      messageApi.error(
        err instanceof Error ? err.message : "No se pudo guardar el objetivo.",
      );
    } finally {
  ...
  const saveSignosVitales = async () => {
    ...
    try {
      ...
      const saved = signosVitalesId
        ? await updateSignosVitales.mutateAsync({
            id: signosVitalesId,
            data: { ...payload, isActive: true },
          })
  ...
  const saveAnalisisDiagnosticosPlan = async () => {
    ...
    if (!analysis.trim()) {
      messageApi.error("El análisis es obligatorio.");
      return;
    }
    if (!plan.trim()) {
      messageApi.error("El plan es obligatorio.");
      return;
    }
    const hasMain = diagnoses.some((item) => item.main && item.cie10Id);
    const hasIncomplete = diagnoses.some(
      (item) => !item.cie10Id || !item.diagnosis,
    );
    if (!hasMain) {
      messageApi.error("Debe registrar un diagnóstico principal.");
      return;
    }
    if (hasIncomplete) {
      messageApi.warning("Complete o elimine los diagnósticos incompletos.");
      return;
    }
    if (!admissionId) {
      messageApi.error(
    ...
    try {
      ...
      const saved = analisisId
        ? await updateAnalisis.mutateAsync({
            id: analisisId,
            data: { ...payload, isActive: true },
          })
      ...
      messageApi.success(
        `Análisis, diagnósticos y plan guardados para ${patientName}.`,
      );
    } catch (err) {
      messageApi.error(
        err instanceof Error
          ? err.message
          : "No se pudo guardar el análisis, diagnósticos y plan.",
      );
    } finally {
...
export type UseHciInicialFormResult = ReturnType<typeof useHciInicialForm>;
