...
function normalizePatient(
  ...
  return {
    ...
    segundoNombre: pick<string>(r, "segundoNombre", "SegundoNombre") ?? null,
    primerApellido: pick<string>(r, "primerApellido", "PrimerApellido") ?? "",
    segundoApellido:
...
async function fetchPatientByDocument(
  doc: string,
): Promise<PatientMiniResponse> {
  ...
  const backendMessage =
    typeof data === "string"
      ? data
      : ((data as { error?: string; message?: string; title?: string })
          ?.error ??
