...
const fetchTypes = async (): Promise<Resource<Type[]>> => {
  ...
  const url =
    `${process.env.NEXT_PUBLIC_POKEMON_LIST_API_BASE_URL}${locale}_types` || "";
  console.log("🧅🧅🧅", url);
  try {
...
const fetchEggGroups = async (): Promise<Resource<EggGroup[]>> => {
  ...
  const url =
    `${process.env.NEXT_PUBLIC_POKEMON_LIST_API_BASE_URL}${locale}_egg_groups` ||
    "";
...
export default async function PokemonForm(props: Props) {
  ...
  return <Form types={types} eggGroups={eggGroups} pokemon={pokemon} />;
}
